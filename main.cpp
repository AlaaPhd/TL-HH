...
double Hyper_heuristic::calculate_diversity12(int* team, int team_size) {
    double diversity = 0;
    for (int j = 0; j < team_size; ++j) {
        for (int k = j + 1; k < team_size; ++k) {
            diversity += div_in[team[j]][team[k]];
        }
    }
    return diversity;
}

int Hyper_heuristic::calculate_efficiency12(int* team, int team_size) {
    int efficiency = 0;
    for (int i = 0; i < team_size; ++i) {
        efficiency += eff[team[i]];
    }
    return efficiency;
}


void Hyper_heuristic::update_delta12(int node, int team_min, int team_old) {
    for (int t = 1; t <= num_team; ++t) {
        if (t != team_min && t != team_old) {
            w_div[team_min] += div_in[node][t];
            w_div[team_old] -= div_in[node][t];
        }
    }
}

double Hyper_heuristic::swap_min12(int iter, int &c1, int &c2) {
    double best = MINVALUE;
    for (int i = 0; i < num_node; ++i) {
        if (state[i] == 0) continue;
        for (int j = 0; j < num_node; ++j) {
            if (state[j] != 0) continue;
            double gain = div_in[i][j] - div_in[i][state[j]];
            if (gain > best) {
                best = gain;
                c1 = i;
                c2 = j;
...
void Hyper_heuristic::repair_solution12() {
    // Step 1: Identify teams below the minimum diversity threshold
    for (int t = 1; t <= num_team; ++t) {
        if (w_div[t] < min_div) {
            // Step 2: For each team below the threshold, try to find better individuals
            bool improved = true;
            while (w_div[t] < min_div && improved) {
                improved = false;
                int best_candidate = -1;
                double best_diversity_increase = 0.0;
                int replace_index = -1;

                // Iterate over unallocated individuals
                for (int u = 0; u < num_node; ++u) {
                    int candidate = team[0][u];
                    if (candidate == -1) continue; // Skip if it's a placeholder or already used

                    for (int m = 0; m < num_each_t; ++m) {
                        int original_member = team[t][m];

                        // Swap in the candidate
                        team[t][m] = candidate;
                        double new_diversity = calculate_diversity12(team[t], num_each_t);

                        double diversity_increase = new_diversity - w_div[t];

                        if (diversity_increase > best_diversity_increase) {
                            best_candidate = candidate;
                            best_diversity_increase = diversity_increase;
                            replace_index = m;
                            improved = true;
                        }

                        // Revert the swap
                        team[t][m] = original_member;
                    }
                }

                // If a beneficial swap was found, perform it
                if (best_candidate != -1) {
                    int original_member = team[t][replace_index];
                    team[t][replace_index] = best_candidate;

                    // Update unallocated pool and team metrics
                    for (int i = 0; i < num_node; ++i) {
                        if (team[0][i] == best_candidate) {
                            team[0][i] = original_member;
                            break;
                        }
                    }

                    w_div[t] = calculate_diversity12(team[t], num_each_t);
                    w_eff[t] = calculate_efficiency12(team[t], num_each_t);
                }
            }
        }
    }

    // Ensure no duplicates in team[0] and clear the used individuals
    std::vector<int> unallocated;
    for (int i = 0; i < num_node; ++i) {
        if (team[0][i] != -1) {
            unallocated.push_back(team[0][i]);
        }
    }
    std::fill(team[0], team[0] + num_node, -1); // Reset team[0]

    for (int i = 0; i < unallocated.size(); ++i) {
        team[0][i] = unallocated[i]; // Fill back the unallocated individuals
    }
}


void Hyper_heuristic::generate_initial123() {
    std::vector<int> individuals(num_node);
    std::iota(individuals.begin(), individuals.end(), 0); // Fill individuals with 0 to num_node-1
    std::random_shuffle(individuals.begin(), individuals.end());
    int unallocated_index = 0;
    for (int t = 0; t < num_team; ++t) {
        for (int m = 0; m < num_each_t; ++m) {
            team[t + 1][m] = individuals[unallocated_index++]; // Assign individuals to teams
        }
    }
    // Store unallocated individuals in team 0
    int unallocated_count = 0;
    for (; unallocated_index < num_node; ++unallocated_index) {
        team[0][unallocated_count++] = individuals[unallocated_index];
    }
    // Allocate memory for team efficiency and diversity
    w_eff = new int[num_team + 1]();
    w_div = new int[num_team + 1]();

    std::vector<int> unallocated_individuals;
    for (int i = 0; i < unallocated_count; ++i) {
        unallocated_individuals.push_back(team[0][i]);
    }
    // Calculate initial efficiency and diversity
    for (int t = 1; t <= num_team; ++t) {
        w_eff[t] = calculate_efficiency12(team[t], num_each_t);
        w_div[t] = calculate_diversity12(team[t], num_each_t);
    }
    // Repair mechanism to meet min_div
    for (int t = 1; t <= num_team; ++t) {
        if (w_div[t] < min_div) {
           // std::cout << "Adjusting Team " << t << " to meet minimum diversity..." << std::endl;

            bool improved = true;
            while (w_div[t] < min_div && improved) {
                improved = false;
                for (int m = 0; m < num_each_t && w_div[t] < min_div; ++m) {
                    int original_member = team[t][m];
                    double best_diversity = w_div[t];
                    int best_member = original_member;

                    for (int candidate : unallocated_individuals) {
                        team[t][m] = candidate;
                        double new_diversity = calculate_diversity12(team[t], num_each_t);
                    ...
                    if (best_member != original_member) {
                        unallocated_individuals.push_back(original_member);
                        team[t][m] = best_member;
                        unallocated_individuals.erase(std::remove(unallocated_individuals.begin(), unallocated_individuals.end(), best_member), unallocated_individuals.end());
                        w_div[t] = best_diversity;
                        w_eff[t] = calculate_efficiency12(team[t], num_each_t); // Update efficiency after change
                    } else {
                        team[t][m] = original_member; // revert if no better swap found
                    }
                }
            }
        }
    ...
    // Remove allocated individuals from the unallocated team
    unallocated_count = 0;
    for (int i = 0; i < num_node; ++i) {
        bool is_allocated = false;
        for (int t = 1; t <= num_team; ++t) {
            if (std::find(team[t], team[t] + num_each_t, i) != team[t] + num_each_t) {
                is_allocated = true;
                break;
            }
        }
        if (!is_allocated) {
            team[0][unallocated_count++] = i;
        }
    }
      //repair_solution12();
      f_cur=min_func(w_eff,num_team);
    cout << "Unallocated Individuals:" << endl;
    cout << "Team " << 0 << ": ";
    for (int m = 0; m < unallocated_count; ++m) {
        cout << team[0][m] << " ";
    }
    cout << endl;
    cout << endl;
    cout << "Final Teams and their Efficiencies and Diversities:" << endl;
    for (int t = 1; t <= num_team; ++t) {
        cout << "Team " << t << ": ";
        for (int m = 0; m < num_each_t; ++m) {
            cout << team[t][m] << "\t";
        }
        cout << "Eff: " << w_eff[t] << "\t" << " Div: " << w_div[t] << endl;
    }
}

int** Hyper_heuristic::swap_LLH1( int &d1, int &d2)
{  //LLH1: Swap individual with minimum efficiency with individual not assigned.
...
int** Hyper_heuristic::swap_LLH2(int &d1, int &d2)
{    //LLH2: Swap individual with minimum efficiency with individual assigned.
	double cha1, cha2, cha;
	int node1, node2, team_min, team_old;
	int a1, a2;
...
int** Hyper_heuristic::swap_LLH3(int **team)
{   //LLH3: Move a randomly selected individual from a randomly selected team to another randomly selected team.

    int team1;
    int team2;
    int **temp;
    srand(time(NULL));
    team1 = rand()% num_team +1;
    team2 = rand()% num_team +1;
    int  ind= rand()% num_each_t+1;
    if(( team1 == team2)&&(team2 > num_team)){
        team2 = rand()% num_team +1;
     }
//    cout<<"----------------------------------------------------"<<endl;
//	cout<<"LLH3:Move a randomly selected individuals from a randomly selected team to another randomly selected team"<<endl;
   // cout<<"Randomly select two teams: "<<"team="<<team1<<" "<<"team="<<team2;
   // cout<<endl;
    //cout<<"Randomly select Individual: "<<"Individual="<<ind;
    //cout<<endl;
   // Swap practitioner between teams
    if ((team1 != team2)){

		int temp1;
		temp1=team[team1][ind];
		team[team1][ind]=team[team2][ind];
		team[team2][ind]=temp1;
        //swap(team[team1][ind],team[team2][ind]);
    }
    repair_solution();
    return team;
}
int** Hyper_heuristic::swap_LLH4(int **team)
{    //LLH4: Swap two randomly selected individuals between two randomly selected teams.
    int team1;
    int team2;
    int **temp;
    srand(time(NULL));
    team1 = rand()% num_team + 1;
    team2 = rand()% num_team + 1;
    int  ind1= rand()% num_each_t;
    int  ind2= rand()% num_each_t;
    if(( team1 == team2)&&(team2 > num_team)){
        team2 = rand()% num_team +1;
     }
      if(( ind1 == ind2)&&(ind2 > num_each_t)){
        team2 = rand()% num_each_t;
     }
//    cout<<"----------------------------------------------------"<<endl;
//	cout<<"LLH4:Swap two randomly selected individuals between two randomly selected teams"<<endl;
    //cout<<"Randomly select two teams: "<<"team="<<team1<<" "<<"team="<<team2;
    //cout<<endl;
    //cout<<"Randomly select Individual: "<<"Individual 1="<<ind1<<"\t"<<"Individual 2="<<ind2;
    //cout<<endl;
      //  w_div[team1] = 0;
		//w_eff[team1] = 0;
		//div_best[team1] = 0;
		...
		//eff_best[team2] = 0;
   // Swap practitioner between teams
    if ((team1 != team2) && (ind1 != ind2)){
		int temp1;
		temp1 = team[team1][ind1];
		team[team1][ind1] = team[team2][ind1];
		team[team2][ind1] = temp1;
        //swap(team[team1][ind],team[team2][ind]);
        int temp2;
		temp2 = team[team1][ind2];
		team[team1][ind2] = team[team2][ind2];
		team[team2][ind2] = temp2;
    }
...
int** Hyper_heuristic::swap_LLH5(int **team)
{   //LLH5: Swap two consecutive individuals between two randomly selected teams.
    int team1;
    int team2;
    int **temp;
    srand(time(NULL));
    team1 = rand()% num_team +1;
    team2 = rand()% num_team +1;
    int d11 =0;// rand() % num_node; // Randomly select practitioner 1 from team 1
    int d12 =1;// rand() % num_node; // Randomly select practitioner 2 from team 1
    int d13 =2;// rand() % num_node; // Randomly select practitioner 3 from team 1
    int d21 =0;// rand() % num_node; // Randomly select practitioner 1 from team 2
    int d22 =1;// rand() % num_node; // Randomly select practitioner 2 from team 2
    int d23 =2;// rand() % num_node; // Randomly select practitioner 3 from team 2
    if(( team1 == team2)&&(team2 > num_team)){
        team2 = rand()% num_team +1;
     }
//      if(( ind1 == ind2)&&(ind2 > num_each_t)){
...
//     }
//    cout<<"----------------------------------------------------"<<endl;
//	cout<<"LLH5:Swap two consecutive selected individuals between two randomly selected teams"<<endl;
    //cout<<"Randomly select two teams: "<<"team="<<team1<<" "<<"team="<<team2;
    //cout<<endl;
    //cout<<" Select first two consecutive Individuals to swap.";
    //cout<<endl;
   // Swap practitioner between teams
    if (team1 != team2){

		int temp1;
		temp1 = team[team1][d11];
		team[team1][d11] = team[team2][d21];
		team[team2][d21] = temp1;
        //swap(team[team1][ind],team[team2][ind]);
        int temp2;
		temp1=team[team1][d12];
		team[team1][d12] = team[team2][d22];
		team[team2][d22] = temp1;
//        int temp3;
//		temp3=team[team1][d13];
//		team[team1][d13]=team[team2][d23];
//		team[team2][d23]=temp3;}
}
    return team;
}
int** Hyper_heuristic::swap_LLH6(int **team)
{   //	LLH6: Swap three consecutive individuals between two randomly selected teams.
...
//    cout<<endl;
   // Swap practitioner between teams
    if (team1 != team2){
		...
		team[team2][d23]=temp3;
        /*for (int j = 0 ; j < num_each_t ; j++){
                w_eff[team1] = w_eff[team1] + eff[team[team1][j]];
                w_eff[team2] = w_eff[team2] + eff[team[team2][j]];
         for(int k = j+1; k <= num_each_t; k++){
          w_div[team1] = w_div[team1] + div_in[team[team1][j]][team[team1][k]];
          w_div[team2] = w_div[team2] + div_in[team[team2][j]][team[team2][k]];
              }
         }*/
...
int** Hyper_heuristic::swap_LLH7(int **team)
{   //LLH6: Swap three consecutive individuals between two randomly selected teams.
    ...
    int **temp;
    srand(time(NULL));
    team1 = rand() % num_team; // Randomly select team 1 from 1 to num_team
    team2 = rand() % num_team; // Randomly select team 2 from 1 to num_team
    int d11 =0;// rand() % num_node; // Randomly select practitioner 1 from team 1
    int d12 =1;// rand() % num_node; // Randomly select practitioner 2 from team 1
    int d13 =2;// rand() % num_node; // Randomly select practitioner 3 from team 1
    int d21 =0;// rand() % num_node; // Randomly select practitioner 1 from team 2
...
//    cout<<endl;
   // Swap practitioner between teams
   if (team1==team2){
        team2 = rand()%num_team; // Randomly select practitioner 1 from team 1
    if (team1 != team2){
        ...
        //swap(team[team1][ind],team[team2][ind]);
        int temp2;
		temp2=team[team1][d12];
		team[team1][d12]=team[team2][d22];
		team[team2][d22]=temp2;
...
int** Hyper_heuristic::PertubILS(int** team){
    // 50%  individuals swapped
    int team1;
    int team2;
    srand(time(NULL));
    team1 = rand()%num_team;
    ...
    for( int t = 1; t <= num_team ; t++){
       if ((t % 2 != 0)&&( t < num_team))
        {
            for(int i = 0; i <= (num_each_t*0.5); i++)
            {
            int temp = team[team1][i];
             team[team1][i] = team[team2][i];
             team[team2][i] = temp;
            }
        }
       }
    ...
    objective_Function(team);
    int idx = min_func(w_eff, num_team);
	f_cur = w_eff[idx];    //objective function of current solution
	//infeasible_local_search();
   repair_solution();
...
int Hyper_heuristic::swap_min_with_z(int iter, int &c1, int &c2)
{
...
// Shift Operator
int Hyper_heuristic::shift(int iter, int &d1, int &d2) {
    double eff_best = MINVALUE;
    int idx_min_eff = min_func(w_eff, num_team);

    // Find a member from the team with minimum efficiency and move to another team
    for (int i = 0; i < num_each_t; ++i) {
        int member = team[idx_min_eff][i];
        for (int j = 0; j < num_team; ++j) {
            if (j != idx_min_eff) {
                double new_eff = w_eff[j] + eff[member];
                if (new_eff > eff_best && w_div[j] + delta_div[member][j] >= min_div) {
                    eff_best = new_eff;
                    d1 = member;
                    d2 = j;
                }
    ...
    // Move the member
    if (eff_best > MINVALUE) {
        state[d1] = d2;
        for (int i = 0; i < num_each_t; ++i) {
            if (team[idx_min_eff][i] == d1) {
                team[idx_min_eff][i] = -1;
                break;
            }
        }
        for (int i = 0; i < num_each_t; ++i) {
...
int Hyper_heuristic::reassignment(int iter, int &d1, int &d2, int reassignment_size = 1) {
    double eff_best = MINVALUE;
    int idx_min_eff = min_func(w_eff, num_team);
    int idx_sec_eff = sec_func(w_eff, idx_min_eff, num_team);

    // Initialize candidate lists using vectors
    vector<int> can_i;
    vector<int> can_j;
    vector<int> tabu_can_i;
    vector<int> tabu_can_j;

    double eff_tabu_best = MINVALUE;

    // Loop to perform multiple reassignments if required
    for (int r = 0; r < reassignment_size; ++r) {
        eff_best = MINVALUE;
        eff_tabu_best = MINVALUE;
        d1 = -1;
        d2 = -1;

        // Iterate over all practitioners to find potential reassignments
        for (int i = 0; i < num_node; ++i) {
            if (state[i] != idx_min_eff && state[i] != 0) {
                double new_eff = w_eff[idx_min_eff] + eff[i];
                // Check if this reassignment offers better efficiency and maintains diversity
                if (new_eff > eff_best && w_div[idx_min_eff] + delta_div[i][idx_min_eff] >= min_div) {
                    // Check tabu status
                    bool is_tabu = (tabu_list[i][idx_min_eff] > iter);
                    ...
                    if (!is_tabu) {
                        if (new_eff > eff_best) {
                            ...
                            can_j.clear();
                            can_i.push_back(i);
                            can_j.push_back(idx_min_eff);
                        }
                        else if (fabs(new_eff - eff_best) <= 1.0e-5 && can_i.size() < MAXNUM) {
                            can_i.push_back(i);
                            can_j.push_back(idx_min_eff);
                        }
                    }
                    else {
                        if (new_eff > eff_tabu_best) {
                            eff_tabu_best = new_eff;
                            tabu_can_i.clear();
                            tabu_can_j.clear();
                            tabu_can_i.push_back(i);
                            tabu_can_j.push_back(idx_min_eff);
                        }
                        else if (fabs(new_eff - eff_tabu_best) <= 1.0e-5 && tabu_can_i.size() < MAXNUM) {
                            tabu_can_i.push_back(i);
                            tabu_can_j.push_back(idx_min_eff);
                        }
                    }
        ...
        // Decide whether to perform a tabu move based on aspiration criterion
        bool perform_tabu = false;
        if (!tabu_can_i.empty() && eff_tabu_best > eff_best) {
            // Additional condition: applying this move improves the global best
            if ((f_cur + eff_tabu_best > fbest)) {
                perform_tabu = true;
            }
        }

        if (perform_tabu && !tabu_can_i.empty()) {
            // Select a random tabu candidate
            int rx = rand() % tabu_can_i.size();
            d1 = tabu_can_i[rx];
            d2 = tabu_can_j[rx];
            eff_best = eff_tabu_best;
            // Optionally, prioritize this move
        }
        else if (!can_i.empty()) {
            // Select a random non-tabu candidate
            int rx = rand() % can_i.size();
            d1 = can_i[rx];
            d2 = can_j[rx];
        }
        else {
            // No feasible reassignment found
            continue;
        }

        // Perform the reassignment
        if (eff_best > MINVALUE && d1 != -1 && d2 != -1) {
            // Remove practitioner d1 from their current team
            for (int j = 0; j < num_each_t; ++j) {
                if (team[state[d1]][j] == d1) {
                    team[state[d1]][j] = -1;
                    break;
                }
            }
            // Add practitioner d1 to the least efficient team (idx_min_eff)
            for (int j = 0; j < num_each_t; ++j) {
                if (team[d2][j] == -1) {
                    team[d2][j] = d1;
                    break;
            ...
            // Update efficiency and diversity metrics
            w_eff[idx_min_eff] += eff[d1];
            w_div[idx_min_eff] += delta_div[d1][idx_min_eff];
            w_eff[state[d1]] -= eff[d1];
            w_div[state[d1]] -= delta_div[d1][state[d1]];

            // Update tabu list
            tabu_list[d1][d2] = iter + tl + randomInt(tabu_tenure);
            tabu_list[d2][state[d1]] = iter + tl + randomInt(tabu_tenure);
        }
    }
...
void Hyper_heuristic::repair_solution_restrict()
{
	int c1 = -1, c2 = -1;
	double min, best;
	int node1, node2, team_min, team_old;
	...
	int idx1 = sec_func(w_div, idx, num_team);
	min = w_div[idx];
	double min1 = w_div[idx1];
	//cout<<idx;
	for (int i = 0; i < num_node; i++)
		for (int j = 0; j <= num_team; j++)
			tabu_list[i][j] = 0;//
	while (min < min_div)
	{
		best = swap_min_with_z(iter, c1, c2);
		node1 = c1;									 /* move in */
		node2 = c2;									 /* move out */
...
int Hyper_heuristic::Greedy_Randomized_list() {
    // ==========================================================
    // Parameters for GRASP-like randomized greedy construction
    // ==========================================================
    double alpha = 0.3; // Controls the RCL size (30%)
    ...
    // ----------------------------------------------------------
    // Step 2: Prepare sorted candidate list (descending efficiency)
    // ----------------------------------------------------------
    std::vector<std::pair<int, int>> candidates;
    candidates.reserve(num_node);
    for (int i = 0; i < num_node; i++)
        candidates.emplace_back(i, eff[i]);
    ...
    // ----------------------------------------------------------
    // Step 3: Assign one candidate per team from RCL
    // ----------------------------------------------------------
    int rcl_size = static_cast<int>(alpha * candidates.size());
    if (rcl_size < num_team) rcl_size = num_team;

    for (int i = 1; i <= num_team; i++) {
        int idx_rcl = randomInt(rcl_size);
        int chosen_node = candidates[idx_rcl].first;

        w_eff[i] += eff[chosen_node];
        num_t_cur[i]++;
        update_delta(chosen_node, i, state[chosen_node]);
        state[chosen_node] = i;

        // Remove chosen node
        std::swap(candidates[idx_rcl], candidates[rcl_size - 1]);
        rcl_size--;
    }

    // ----------------------------------------------------------
    // Step 4: Fill remaining teams using greedy randomized selection
    // ----------------------------------------------------------
    while (true) {
        // Check if all teams are filled
        bool all_filled = true;
        for (int i = 1; i <= num_team; i++) {
            if (num_t_cur[i] < num_each_t) {
                all_filled = false;
                break;
            }
        }
        if (all_filled || candidates.empty()) break;

        // Remove already assigned nodes
        candidates.erase(std::remove_if(candidates.begin(), candidates.end(),
            [&](const auto &c) { return state[c.first] != 0; }), candidates.end());
        if (candidates.empty()) break;

        // Sort again after removals
        std::sort(candidates.begin(), candidates.end(),
                  [](const auto &a, const auto &b) {
                      return a.second > b.second;
                  });
        ...
        // Define RCL window
        rcl_size = static_cast<int>(alpha * candidates.size());
        if (rcl_size < 1) rcl_size = 1;

        // Pick random candidate from RCL
        int idx_rcl = randomInt(std::min(rcl_size, static_cast<int>(candidates.size())));
        int chosen_node = candidates[idx_rcl].first;

        // Pick random feasible team
        std::vector<int> feasible_teams;
        for (int i = 1; i <= num_team; i++)
            if (num_t_cur[i] < num_each_t)
                feasible_teams.push_back(i);

        if (feasible_teams.empty()) break;

        int chosen_team = feasible_teams[randomInt(feasible_teams.size())];

        // Assign node
        w_eff[chosen_team] += eff[chosen_node];
        num_t_cur[chosen_team]++;
        update_delta(chosen_node, chosen_team, state[chosen_node]);
        state[chosen_node] = chosen_team;

        candidates.erase(candidates.begin() + idx_rcl);
    }

    // ----------------------------------------------------------
    // Step 5: Construct team matrix
    // ----------------------------------------------------------
    for (int i = 0; i <= num_team; i++)
        for (int j = 0; j < num_node; j++)
            team[i][j] = -1;
...
int** Hyper_heuristic::GRASP() {
    ...
    int f_Sinitial, f_Sbest, f_S;
    int** Sinitial = nullptr;
    int** Sbest = nullptr;
    int** S = nullptr;
    ...
    std::filesystem::path conv_path = folder_path / "GRASP_Convergence.csv";
    std::ofstream conv_file(conv_path);
    if (conv_file.is_open()) {
        conv_file << "Iteration,Fbest\n";
    }
    ...
    // ==========================================================
    // Step 1: Initial Construction (Greedy Randomized)
    // ==========================================================
    generate_initialrandom();     // creates a randomized greedy solution
    objective_Function1(team);
    Sinitial = team;
    f_Sinitial = f_cur;

    // Initialize best solution
    Sbest = deep_copy_solution(Sinitial, num_node, num_team, num_each_t);
    f_Sbest = f_Sinitial;

    // Apply local search to improve initial solution
    f_S = feasible_local_search();
    S = team;

    if (f_S > f_Sbest) {
        free_solution(Sbest, num_node, num_team, num_each_t);
        Sbest = deep_copy_solution(S, num_node, num_team, num_each_t);
        f_Sbest = f_S;
    }

    convergence.push_back(f_Sbest);
    if (conv_file.is_open()) conv_file << iter << "," << f_Sbest << "\n";

    // ==========================================================
    // Main GRASP Iterations
    // ==========================================================
    while (iter < max_iter) {
        // Step 1: Construct new randomized greedy solution
        generate_initial();
        objective_Function1(team);
        Sinitial = team;
        f_Sinitial = f_cur;
...
int Hyper_heuristic::iterated_local_search12() {
    int ls_depth = 300; // Number of iterations
    int iter = 0;
    int f_Sintial, f_Sbest, f_S, f_Spar2;
    double best;
    int node1, node2, team_min, team_old;
    int a1, a2, d1 = 0, d2 = 0;

    int** Sinitial = team;  // initial candidate solution
    int** Sbest = Sinitial; // best solution
    f_Sintial = f_cur;
    f_Sbest = f_Sintial;

    f_S = feasible_local_search(); // apply local search on initial solution
    int** S = team;
    ...
    std::filesystem::path conv_path = folder_path / "iterated_local_search12_Convergence.csv";
    std::ofstream conv_file(conv_path);
    if (conv_file.is_open()) {
        conv_file << "Iteration,Fbest\n";
    }
    ...
    while (iter < ls_depth) {
        ...
        if ((f_Spar2 > f_S) && (f_Spar2 > f_Sbest)) {
            end_time = clock();
            f_S = f_Spar2;
            f_Sbest = f_Spar2;
            S = Spar2;
